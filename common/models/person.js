...
module.exports = function(Person) {
  ...
  Person.prototype.exportResume = function(email,onlyPreview,res,next){
    var personId = this.id
    Person.findById(this.id,{
      'include': ['certificate','education','experience','profile','project','awards']
    }, function(err,person){
      if(err) next(err);
      else{
        var info ={};
        info['host'] =  Person.app.get('host');
        info['port'] =  Person.app.get('port');
        info['name'] = person.fname+" "+person.lname;
        info['pic'] = person.pic;

        if(person.contacts && person.contacts.length){
          person.contacts.forEach(function(contact){
            if(contact.category == 'Phone') info['phone'] = contact.contact;
            if(contact.category == 'Email') info['email'] = contact.contact;
          });
        }
        ...
        var cvTempPath = Person.app.get('cvTempPath');
        res.render(cvTempPath, {
          info: info
        }, function (err, HTML) {
          if(err) next(err);
          else if(onlyPreview){
            console.log(Person.app.middleware)
            next(null,{HTML})
          }else{
            //create pdf in storage directory
            pdf.create(HTML, options).toFile(Person.app.datasources.storage.settings.root+'/media'+personId+'/resume.pdf', function (err, result) {
              if (err) next(err);
              else{
                if(email){
                  //mail resume 
                ...
                //res.send(HTML);
                var path = 'http://'+Person.app.get('host')+':'+Person.app.get('port')+'/api/containers/media'+personId+'/download/resume.pdf';
                next(null,{HTML,path})
              }
            })
          }
  ...
  Person.prototype.uploadMedia = function(req,next){
    var id = this.id;
    var baseUrl = 'http://'+Person.app.get('host')+':'+ Person.app.get('port')+'/api/containers/';
